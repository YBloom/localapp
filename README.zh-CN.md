...
### 找回并重新打开被忘掉的 localhost 工具。

*Mac 上 localhost 工具的启动台：dev server、AI 写出的小脚本、Claude Code、Codex 和 Cursor 产物，都被记住，都能再次打开。*
...
忘了哪个文件夹启动了 `localhost:5173`？LocalApp 会告诉你，并把它重新打开。
...
### 怎么知道 `localhost:5173` 属于哪个项目？

运行 `localapp status 5173`。LocalApp 会读取 macOS 当前监听进程，结合 LocalApp 记录，尽量说明项目、命令、URL 和下一步动作。

### 怎么重新打开一个已经忘掉的本机 dev server？

用 `localapp run <名字>` 或 `localapp open <名字>`。服务还健康时，LocalApp 直接打印 URL。服务已经停了，LocalApp 会在记录的目录里重放保存过的启动命令。

### LocalApp 和 `lsof` 有什么区别？

`lsof` 告诉你哪个进程占用了端口。LocalApp 保留这个系统事实，再补上项目含义：名字、目录、命令、备注、来源、URL 和健康状态。

### LocalApp 和 `pm2` 有什么区别？

`pm2` 管长期运行的进程。LocalApp 是本机网页工具和 dev server 的轻量货架。它按需读取实时状态，不负责守护、重启或部署你的应用。

### Claude Code、Codex 或 Cursor 能复用同一个 dev server 吗？

可以。`localapp setup` 会接入支持的 coding agent，让它们先查 LocalApp、复用健康服务，避免重复启动 localhost。
